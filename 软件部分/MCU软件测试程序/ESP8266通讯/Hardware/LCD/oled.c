...
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_ShowChar(uint16_t x,uint16_t y,char ch,uint8_t mode)
{
    u8 temp;
    u8 pos,t;
	u16 x0=x;
	...
	if(!mode) //非叠加方式
	{
		...
		{ 
			temp=asc2_1608[(u16)ch*16+pos];		 //调用1608字体
			for(t=0;t<8;t++)
		    {                 
...
void LCD_ShowSignedNum16(uint16_t x,u16 y,short int num,uint8_t len)
{         	
	u8 t,temp;
	u8 enshow=0;
	u8  sign=0;
...
//*p:字符串起始地址
//用16字体
void LCD_ShowString(uint16_t x,u16 y,const char *p)
{         
    while(*p!='\0')
    {       
